...
void serverManager::setup(){
	...
	// Subscribe to VST events from server and forward them
	listeners.push(server->vstParameterChanged.newListener(this, &serverManager::onVSTParameterChanged));
	listeners.push(server->vstParameterAutomated.newListener(this, &serverManager::onVSTParameterAutomated));
	listeners.push(server->vstPluginOpened.newListener([this](VSTPluginOpenedEvent& event) {
		vstPluginOpened.notify(event);
	}));
}

void serverManager::onVSTParameterChanged(VSTParameterEvent& event) {
	// Forward the event to any listening nodes
	vstParameterChanged.notify(event);
}

void serverManager::onVSTParameterAutomated(VSTParameterEvent& event) {
	// Forward the event to any listening nodes
